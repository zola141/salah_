...
export default function Game({ players, bots, gameType, multiplayer }) {
  ...
  useEffect(() => {
    // Only setup chat messages in multiplayer mode
    if (!multiplayer || !chatSocket) return;
  ...
  }, [multiplayer, chatSocket]);

  useEffect(() => {
    // Only initialize chat socket for multiplayer games
  ...
  useEffect(() => {
    // Only setup chat room for multiplayer games
    if (!multiplayer) return;
  ...
  useEffect(() => {
    // Only join chat in multiplayer mode
    if (!multiplayer) return;
  ...
  useEffect(() => {
    // Only fetch chat lists in multiplayer mode
    if (!multiplayer) return;
  ...
  useEffect(() => {
    // Only setup DM listener in multiplayer mode
    if (!multiplayer || !chatSocket) return;
  ...
  }, [multiplayer, chatSocket, dmTarget]);
        ...
        <div className="dice-control">
          ...
          <button
            ...
            disabled={
              ...
              (!multiplayer && BOT_PLAYERS.includes(PLAYERS[activePlayer]))
            }
            onClick={() => {
